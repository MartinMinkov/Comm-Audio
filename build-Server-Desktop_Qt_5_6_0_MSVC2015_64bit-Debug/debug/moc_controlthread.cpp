/****************************************************************************
** Meta object code from reading C++ file 'controlthread.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.6.0)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../Server/controlthread.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'controlthread.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.6.0. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_ControlThread_t {
    QByteArrayData data[8];
    char stringdata0[75];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    qptrdiff(offsetof(qt_meta_stringdata_ControlThread_t, stringdata0) + ofs \
        - idx * sizeof(QByteArrayData)) \
    )
static const qt_meta_stringdata_ControlThread_t qt_meta_stringdata_ControlThread = {
    {
QT_MOC_LITERAL(0, 0, 13), // "ControlThread"
QT_MOC_LITERAL(1, 14, 11), // "signalSetup"
QT_MOC_LITERAL(2, 26, 0), // ""
QT_MOC_LITERAL(3, 27, 4), // "port"
QT_MOC_LITERAL(4, 32, 16), // "signalDisconnect"
QT_MOC_LITERAL(5, 49, 8), // "finished"
QT_MOC_LITERAL(6, 58, 5), // "setup"
QT_MOC_LITERAL(7, 64, 10) // "disconnect"

    },
    "ControlThread\0signalSetup\0\0port\0"
    "signalDisconnect\0finished\0setup\0"
    "disconnect"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_ControlThread[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       3,       // signalCount

 // signals: name, argc, parameters, tag, flags
       1,    1,   39,    2, 0x06 /* Public */,
       4,    0,   42,    2, 0x06 /* Public */,
       5,    0,   43,    2, 0x06 /* Public */,

 // slots: name, argc, parameters, tag, flags
       6,    1,   44,    2, 0x0a /* Public */,
       7,    0,   47,    2, 0x0a /* Public */,

 // signals: parameters
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void,
    QMetaType::Void,

 // slots: parameters
    QMetaType::Void, QMetaType::Int,    3,
    QMetaType::Void,

       0        // eod
};

void ControlThread::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        ControlThread *_t = static_cast<ControlThread *>(_o);
        Q_UNUSED(_t)
        switch (_id) {
        case 0: _t->signalSetup((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 1: _t->signalDisconnect(); break;
        case 2: _t->finished(); break;
        case 3: _t->setup((*reinterpret_cast< int(*)>(_a[1]))); break;
        case 4: _t->disconnect(); break;
        default: ;
        }
    } else if (_c == QMetaObject::IndexOfMethod) {
        int *result = reinterpret_cast<int *>(_a[0]);
        void **func = reinterpret_cast<void **>(_a[1]);
        {
            typedef void (ControlThread::*_t)(int );
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&ControlThread::signalSetup)) {
                *result = 0;
                return;
            }
        }
        {
            typedef void (ControlThread::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&ControlThread::signalDisconnect)) {
                *result = 1;
                return;
            }
        }
        {
            typedef void (ControlThread::*_t)();
            if (*reinterpret_cast<_t *>(func) == static_cast<_t>(&ControlThread::finished)) {
                *result = 2;
                return;
            }
        }
    }
}

const QMetaObject ControlThread::staticMetaObject = {
    { &QObject::staticMetaObject, qt_meta_stringdata_ControlThread.data,
      qt_meta_data_ControlThread,  qt_static_metacall, Q_NULLPTR, Q_NULLPTR}
};


const QMetaObject *ControlThread::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *ControlThread::qt_metacast(const char *_clname)
{
    if (!_clname) return Q_NULLPTR;
    if (!strcmp(_clname, qt_meta_stringdata_ControlThread.stringdata0))
        return static_cast<void*>(const_cast< ControlThread*>(this));
    return QObject::qt_metacast(_clname);
}

int ControlThread::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QObject::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 5)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 5;
    }
    return _id;
}

// SIGNAL 0
void ControlThread::signalSetup(int _t1)
{
    void *_a[] = { Q_NULLPTR, const_cast<void*>(reinterpret_cast<const void*>(&_t1)) };
    QMetaObject::activate(this, &staticMetaObject, 0, _a);
}

// SIGNAL 1
void ControlThread::signalDisconnect()
{
    QMetaObject::activate(this, &staticMetaObject, 1, Q_NULLPTR);
}

// SIGNAL 2
void ControlThread::finished()
{
    QMetaObject::activate(this, &staticMetaObject, 2, Q_NULLPTR);
}
QT_END_MOC_NAMESPACE
